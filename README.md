...
- [x] Fetches data from a real public API
- [x] Uses `useEffect` to load data automatically on mount
- [x] Includes at least one button that triggers a new fetch
- [x] Handles and displays a loading state while data is being fetched
- [x] Renders the fetched data on screen in a clear, readable format

### Code

- [x] Written in React using functional components
- [x] State managed with `useState`
- [x] No console errors on load or interaction

### Styling

- [x] Application is styled with intention — not a blank page with text on it
...
- [x] README.md exists in the root of the project
...
Beach Side & Tides Ocean Resort is a functional React application that displays live coastal weather and ocean condition data for popular beach destinations. The app fetches current and hourly forecast data from a public API, includes a loading state, and allows users to refresh data with a button.
...
### One Technical Challenge and Resolution

Challenge: Open-Meteo returns hourly forecast values as separate arrays, so the app needed to find the correct starting index for the current hour to avoid showing stale data.

Resolution: I used `findIndex` on the hourly time array to locate the first timestamp greater than or equal to the current time, then built the next 6-hour surf brief from that index so the displayed data is always current.
